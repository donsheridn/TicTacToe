...
class TicTacToe
{
    ...
    function DisplayGame()
    {
        if (!$this->isOver()) {         //call the isOver() function to see if any game ending conditions have been met.
            ...
            for ($x = 0; $x < 3; $x++) {
                for ($y = 0; $y < 3; $y++) {
                    ...
                    if ($this->boardvert[$x][$y]) {         //check to see if that position already has an image
                        ...
                        echo "<img src=\"assets/images/{$this->boardvert[$x][$y]}.jpg\" alt=\"{$this->boardvert[$x][$y]}\" title=\"{$this->boardvert[$x][$y]}\" />";
            ...
            echo "<p align=\"center\">
                        <br>
                        <input type=\"submit\" name=\"move\" value=\"Take Turn\" /><br/><br/>
